...
function messageFor(err: unknown): string {
  if (err instanceof XmlParseError || err instanceof EdmxParseError) return err.message;
  ...
  persist(
    (set, get) => ({
      ...
      parse: () => {
        const { xml } = get();
        if (!xml.trim()) {
          set({ parseError: "Paste or upload an EDMX document first." });
          return;
        }
        try {
          ...
          set({ parseError: messageFor(err), entities: [], graph: EMPTY_GRAPH });
        }
      },
