...
function propertyLine(p: EntityProperty): string {
  const flags = [p.isKey ? "key" : null, p.nullable ? "nullable" : "not nullable"].filter(Boolean);
...
function buildPrompt({ entity, incoming }: SummarizeRequest): string {
  const lines: string[] = [
    ...
    "",
    "Properties:",
    ...(entity.properties.length ? entity.properties.map(propertyLine) : ["- (none)"]),
    "",
    "Outgoing relationships:",
    ...(entity.relationships.length ? entity.relationships.map(outgoingLine) : ["- (none)"]),
    "",
...
function isValidPayload(body: unknown): body is SummarizeRequest {
  ...
  if (typeof e.name !== "string" || !Array.isArray(e.properties) || !Array.isArray(e.relationships))
    return false;
