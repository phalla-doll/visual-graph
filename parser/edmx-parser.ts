...
function extractSchemas(edmx: Unknown): Unknown[] {
  const services = asObject(edmx["edmx:DataServices"]) ?? asObject(edmx["DataServices"]);
  if (!services) return [];
  const schemas = services["Schema"] ?? services["schema"];
  return toArray(schemas).map(asObject).filter((s): s is Unknown => s !== null);
}
...
function buildProperties(entityType: Unknown, keys: Set<string>): EntityProperty[] {
...
export function extractEntities(parsed: unknown): Entity[] {
  ...
  if (!edmx) {
    throw new EdmxParseError("No <edmx:Edmx> root found — is this an EDMX document?");
  }
  ...
  for (const schema of schemas) {
    ...
    const types = toArray(schema["EntityType"]).map(asObject).filter((t): t is Unknown => t !== null);
