    ...
    >
      ...
      <div className="divide-y text-xs">
        {data.properties.length === 0 && (
          <div className="px-3 py-1.5 text-muted-foreground italic">(no properties)</div>
        )}
        {data.properties.map((p) => (
          <div key={p.name} className="flex items-center justify-between gap-2 px-3 py-1.5">
            <div className="flex min-w-0 items-center gap-1.5">
              ...
              <span className={cn("truncate", p.isKey && "font-medium")}>{p.name}</span>
            </div>
