...
function nodeHeight(node: GraphNode): number {
  return Math.min(NODE_HEIGHT_BASE + node.properties.length * ROW_HEIGHT, MAX_NODE_HEIGHT);
}
...
export function layoutGraph(
  ...
  g.setGraph({ rankdir: direction, nodesep: 60, ranksep: 100, marginx: 24, marginy: 24 });
  ...
  const rfEdges: ReactFlowEdge<RelationshipEdgeData>[] = edges.map((edge) => ({
    id: edge.id,
    type: "relationship",
    source: edge.source,
    target: edge.target,
